...
object MetricConfiguration{
  ...
  lazy val metricsPath = config.getString("codacy.metric.paths.metrics").getOrElse("/metrics")
  lazy val healthPath  = config.getString("codacy.metric.paths.health" ).getOrElse("/health")
